...
class QTN_PW_EXPORT QtnSubItemEvent: public QEvent
{
public:
    enum Type
    {
...
public:
    QtnDrawContext(QStylePainter* painter_, const QtnPropertyView* widget_, const QRect rect_,
        ...
        :painter(painter_), widget(widget_), rect(rect_), margins(margins_), splitPos(splitPos_),
        isActive(isActive_), hasChildren(hasChildren_)
    {}
